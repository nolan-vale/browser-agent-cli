...
The `chrome-devtools` CLI is the standard tool for browser automation on this machine.

**Stack:** `chrome-beta-agent <url>` opens Chrome Beta → `chrome-devtools` controls it via CDP on `http://127.0.0.1:9222`.

## Prerequisites

Chrome Beta must be running. Start it with:
```bash
chrome-beta-agent <url>
```

The `chrome-devtools` daemon auto-connects to Chrome Beta on port 9222. If it was not recently started, restart it explicitly:
```bash
chrome-devtools start --browserUrl http://127.0.0.1:9222
```

If CDP is flaky, reset only the dedicated browser profile and restart the daemon:
```bash
chrome-beta-agent-stop
chrome-devtools stop || true
chrome-beta-agent <url>
chrome-devtools start --browserUrl http://127.0.0.1:9222
```

The local `chrome-beta-agent` wrapper launches Chrome Beta through macOS `open -na "Google Chrome Beta" --args ...`; this is intentional because directly `nohup`-launching the Chrome binary can leave CDP alive only briefly after the shell exits.

## AI Workflow

1. **Execute**: Run tools directly (e.g., `chrome-devtools list_pages`). The background daemon starts implicitly if already configured; otherwise run `chrome-devtools start --browserUrl http://127.0.0.1:9222` first.
...
Only needed if the daemon lost connection to Chrome Beta (e.g. after a browser restart):
